        ...
        if not user_db.check_user(user_id):
            ...
            if address is None or amount is None:
                ...
                embed.set_author(
                    ...
                    icon_url=ctx.author.avatar_url_as(format='png', size=256))
                embed.add_field(
                    name="No address or amount specified. Please check **!help** for information.",
                    ...
                    if not validate['isvalid']:
                        ...
                        if len(txid) == 64:
                            ...
                            embed = discord.Embed(
                                ...
                                url='https://explorer.umbru.io/insight/tx/{0}'.format(txid), color=0x7152b6)
                            embed.set_author(
                                name=ctx.author.display_name,
                                icon_url=ctx.author.avatar_url_as(format='png', size=256))
                            embed.add_field(
                                ...
                                value="Your Balance: `{0:.6f} UMBRU`".format(client.getbalance(account, config.CONFIRM)))
                            embed.set_footer(text="TipBot v{0}".format(config.VERSION), icon_url=self.bot.user.avatar_url_as(format='png', size=256))
